...
extern uint32_t _estack; // Defined in the linker script by me for RAM1 end

extern uint32_t _sram2_start; // Defined in the linker script by me for SRAM2 start
extern uint32_t _sram2_end; // Defined in the linker script by me for SRAM2 end
...
const char msg_ram2Dump_header1[]     ="[######]==============[RAM2 STATUS]============[######]\r\n";
const char msg_ram2Dump_ram2_header[] ="[.SRAM2] 0x";
const char msg_ram2Dump_ram2_footer[] =" B [.SRAM2]\r\n";
...
// .data section
	...
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"%03lu %03lu",section_size/1000,(section_size%1000));
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
...
//heap section
	...
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"%04lX_%04lX ",(uint32_t)&_end>>16,(uint32_t)&_end&0xFFFF);
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_seperator,strlen(msg_ramDump_seperator),MEMINFO_UART_TIMEOUT);
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"0x%04lX_%04lX ",(uint32_t)&_heap_start>>16,(uint32_t)&_heap_start&0xFFFF);
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_seperator,strlen(msg_ramDump_seperator),MEMINFO_UART_TIMEOUT);
	section_size=(uint32_t)&_heap_start-(uint32_t)&_end;
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"%03lu %03lu",section_size/1000,(section_size%1000));
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
...
// stack section
	...
	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_stack_header,strlen(msg_ramDump_stack_header),MEMINFO_UART_TIMEOUT);
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"%04lX_%04lX ",(uint32_t)MSP>>16,(uint32_t)MSP&0xFFFF);
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_seperator,strlen(msg_ramDump_seperator),MEMINFO_UART_TIMEOUT);
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"0x%04lX_%04lX ",(uint32_t)&_estack>>16,(uint32_t)&_estack&0xFFFF);
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_seperator,strlen(msg_ramDump_seperator),MEMINFO_UART_TIMEOUT);
	section_size=(uint32_t)&_estack-(uint32_t)MSP;
	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"%03lu %03lu",section_size/1000,(section_size%1000));
	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_stack_footer,strlen(msg_ramDump_stack_footer),MEMINFO_UART_TIMEOUT);
...
// // Send RAM2 dump header
// 	HAL_UART_Transmit(&uart,(uint8_t*)msg_ram2Dump_header1,strlen(msg_ramDump_header1),MEMINFO_UART_TIMEOUT);
// 	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_header2,strlen(msg_ramDump_header1),MEMINFO_UART_TIMEOUT);
// // .sram2 section
// 	HAL_UART_Transmit(&uart,(uint8_t*)msg_ram2Dump_data_header,strlen(msg_ram2Dump_data_header),MEMINFO_UART_TIMEOUT);
// 	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"%04lX_%04lX ",(uint32_t)&_sram2_start>>16,(uint32_t)&_sram2_start&0xFFFF);
// 	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
// 	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_seperator,strlen(msg_ramDump_seperator),MEMINFO_UART_TIMEOUT);
// 	snprintf(buffer,MEMINFO_HEX_BUFFER_SIZE,"0x%04lX_%04lX ",(uint32_t)&_sram2_end>>16,(uint32_t)&_sram2_end&0xFFFF);
// 	HAL_UART_Transmit(&uart,(uint8_t*)buffer,strlen(buffer),MEMINFO_UART_TIMEOUT);
// 	HAL_UART_Transmit(&uart,(uint8_t*)msg_ramDump_seperator,strlen(msg_ramDump_seperator),MEMINFO_UART_TIMEOUT);
// 	section_size=(uint32_t)&_sram2_end-(uint32_t)&_sram2_start;
